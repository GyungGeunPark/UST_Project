...
def _snapshot_vr_runtime() -> Dict[str, bool]:
    """Probe live OS state for SteamVR / Oculus runtime processes.

    Returns a dict with keys:
        ``vrserver``        — SteamVR vrserver.exe is running.
        ``ovr_server``      — Oculus/Meta OVRServer_x64.exe is running
                               (likely IPC namespace competitor).
        ``ovr_launcher``    — Oculus service sentinel (auto-respawns
                               OVRServer_x64; must be disabled to keep it dead).
        ``pico_connect``    — PICO Connect is streaming.

    Best-effort: returns ``False`` for everything if psutil is missing or
    the call fails (no diagnosis is better than wrong diagnosis).
    """
    state = {
        "vrserver": False,
        "ovr_server": False,
    ...
    try:
        for proc in psutil.process_iter(["name"]):
            ...
            if name == "vrserver.exe":
                state["vrserver"] = True
            elif name == "ovrserver_x64.exe":
                state["ovr_server"] = True
            elif name in ("ovrservicelauncher.exe", "ovrservice.exe"):
                state["ovr_launcher"] = True
            elif name == "pico connect.exe":
                state["pico_connect"] = True
    except (psutil.Error, OSError):
        pass
    return state


def _format_openvr_init_failure(
    err_name: str,
    exc: BaseException,
    snapshot: Dict[str, bool],
) -> str:
    """Format an actionable diagnostic message for openvr.init() failures.

    Walks ``err_name`` against the standard OpenVR InitError categories and
    cross-references the live runtime snapshot so the message names the
    actual likely culprit (e.g. "OVRServer_x64 is running AND vrserver is
    running -> IPC namespace fight") instead of dumping all three causes.
    """
    ipc_namespace = "IPC_NamespaceUnavailable" in err_name
    ipc_connect = "IPC_ConnectFailed" in err_name
    ipc_compositor = "IPC_CompositorInitFailed" in err_name
    ipc_path = "PathRegistryNotFound" in err_name

    lines = [
        f"[SteamVRSampler] openvr.init() failed: {err_name}: {exc}",
        "",
        "  Live runtime snapshot:",
        f"    vrserver.exe       : {'RUNNING' if snapshot['vrserver'] else 'not running'}",
        f"    OVRServer_x64.exe  : {'RUNNING (Oculus runtime)' if snapshot['ovr_server'] else 'not running'}",
        f"    OVRServiceLauncher : {'RUNNING (auto-respawns OVR)' if snapshot['ovr_launcher'] else 'not running'}",
        f"    PICO Connect.exe   : {'RUNNING' if snapshot['pico_connect'] else 'not running'}",
        "",
    ]

    if ipc_path:
        lines += [
            "  SteamVR install path is not registered in openvrpaths.vrpath.",
            "  Launch SteamVR from Steam once so it writes the path, then retry.",
        ]
        return "\n".join(lines)

    if ipc_namespace or ipc_connect or ipc_compositor:
        lines.append("  Diagnosis:")
        if not snapshot["vrserver"]:
            lines += [
                "    SteamVR vrserver is NOT running.  Open PICO Connect and start",
                "    PCVR streaming -- the prism driver will spawn vrserver.",
                "    Wait until SteamVR's status window shows 'Ready', then retry.",
            ]
        elif snapshot["ovr_server"] or snapshot["ovr_launcher"]:
            lines += [
                "    Oculus/Meta runtime is competing for the OpenVR IPC namespace.",
                "    PICO-only setups do not need it.  Run cleanup_vr_env.py to",
                "    disable the OVR service sentinel and stop OVRServer_x64:",
                "",
                "      python -X utf8 -m ust_ws.ust_hm_grip.scripts.cleanup_vr_env",
                "",
                "    (Run from an *Administrator* PowerShell so the OVR service",
                "    can be stopped + disabled.  Without admin, only OVRServer_x64",
                "    is killed and the launcher will respawn it within seconds.)",
                "    Then retry teleop.",
            ]
        else:
            lines += [
                "    vrserver is running but its IPC namespace is stale (a previous",
                "    OpenVR client crashed without releasing it).  Close SteamVR",
                "    (taskbar icon -> Exit SteamVR), wait until vrserver disappears",
                "    from Task Manager, then re-stream from PICO Connect.  Or run:",
                "",
    ...
    # Unknown OpenVR error code — fall back to the catch-all advice.
    lines += [
        "  Unrecognized OpenVR init failure.  Run the layered diagnostic:",
...
class SteamVRSampler:
    ...
    def start(self) -> None:
        ...
        except openvr.error_code.OpenVRError as exc:
            # ``openvr.init()`` raises when SteamVR's IPC namespace can't be
            # claimed.  Build a runtime diagnosis from live process state so
            # the message names the actual culprit instead of listing all
            # three theoretical causes blindly.
            err_name = type(exc).__name__
            snapshot = _snapshot_vr_runtime()
            raise RuntimeError(_format_openvr_init_failure(err_name, exc, snapshot)) from exc
