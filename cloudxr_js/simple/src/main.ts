...
class CloudXRClient {
  ...
  private async initializeWebGL(): Promise<void> {
    ...
    const gl = webglCanvas.getContext('webgl2', {
      ...
      antialias: kPerformanceOptions.webglContext_antialias,
      failIfMajorPerformanceCaveat: true,
      powerPreference: 'high-performance', // Favor higher power rendering, e.g., discrete GPU over integrated GPU on laptops
  ...
  private async createXRSession(
    ...
    this.baseLayer = new XRWebGLLayer(this.xrSession, this.gl!, {
      alpha: true,
      antialias: kPerformanceOptions.xrWebGLLayer_antialias,
      depth: true,
      framebufferScaleFactor: kPerformanceOptions.xrWebGLLayer_framebufferScaleFactor,
    ...
    if ('fixedFoveation' in this.baseLayer) {
      ...
      console.info('Setting local render foveation to MEDIUM');
      this.baseLayer.fixedFoveation = kPerformanceOptions.xrWebGLLayer_fixedFoveationLevel;
    } else {
  ...
  private async createCloudXRSession(
    ...
    const sessionOptions: CloudXR.SessionOptions = {
      ...
      telemetry: {
        enabled: true,
        appInfo: { version: '6.0.1-beta', product: 'CloudXR.js WebGL Example' },
      },
    };
